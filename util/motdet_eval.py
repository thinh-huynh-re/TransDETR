...
def motdet_evaluate(model, data_loader, iou_thres=0.5, print_interval=10):
    ...
    for batch_i, data in enumerate(data_loader):
        ...
        if target_boxes.size(0) == 0:
            ...
            detected = set()
            print(
                "output_boxes.shape={} class.shape={}".format(
                    outputs_boxes.shape, outputs_class.shape
                )
            )
            ...
            for *pred_bbox, conf in zip(outputs_boxes, outputs_class):
                ...
                if (
                    ...
                    and obj_pred == int(target_cls[best_i])
                    and best_i.item() not in detected
                ):
                    correct.append(1)
                    if conf.sigmoid() > 0.5:
                        num_tp += 1
            ...
            print(
                "precision={} recall={}".format(
                    num_tp / max(1.0, num_dt), num_tp / max(1.0, len(target_boxes))
                )
            )
...
def detmotdet_evaluate(model, data_loader, device, iou_thres=0.5, print_interval=10):
    ...
    for batch_i, data in enumerate(data_loader):
        ...
        for i in range(num_frames):
            ...
            # Means of all images
            metrics_i["mean_mAP"] = np.sum(metrics_i["mAPs"]) / (
                metrics_i["AP_accum_count"] + 1e-16
            )
            metrics_i["mean_R"] = np.sum(metrics_i["mR"]) / (
                metrics_i["AP_accum_count"] + 1e-16
            )
            metrics_i["mean_P"] = np.sum(metrics_i["mP"]) / (
                metrics_i["AP_accum_count"] + 1e-16
            )
