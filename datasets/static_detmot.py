...
class DetMOTDetection:
    ...
    def _pre_single_frame(self, idx: int):
        ...
        obj_idx_offset = (
            self.video_dict[video_name] * 100000
        )  # 100000 unique ids is enough for a video.
        targets["boxes"] = []
        targets["area"] = []
        targets["iscrowd"] = []
        targets["labels"] = []
    ...
    if image_set == "train":
        ...
        if args.cj:
            ...
            color_transforms.append(
                T.MoTColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0)
            )
        if not args.crop:
            scale_transforms = [
...
def build(image_set, args):
    ...
    if image_set == "train":
        ...
        dataset = DetMOTDetection(
            args, data_txt_path=data_txt_path, seqs_folder=root, transforms=transforms
        )
    if image_set == "val":
