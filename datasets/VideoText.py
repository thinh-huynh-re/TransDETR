...
def cv2AddChineseText(image, text, position, textColor=(0, 0, 0), textSize=30):
    ...
    if isinstance(image, np.ndarray):  # 判断是否OpenCV图片类型
        image = Image.fromarray(
            cv2.cvtColor(np.uint8(image.astype(np.float32)), cv2.COLOR_BGR2RGB)
        )
    # 创建一个可以在给定图像上绘图的对象
...
class DetMOTDetection:
    def __init__(self, args, data_txt_path: str, seqs_folder, dataset2transform):
        ...
        if "BOVText" in data_txt_path:
            self.label_files = [
                (
                    (
                        "/share/wuweijia/Data/VideoText/MOTR/BOVText/labels_with_ids/train"
                        + x.split("/Frames")[1]
                    )
                    .replace(".png", ".txt")
                    .replace(".jpg", ".txt")
            ...
            self.voc, self.char2id, self.id2char = get_vocabulary(
                "CHINESE", use_ctc=self.use_ctc
            )
        elif "Synthetic_Chinese_OCR" in data_txt_path:
            self.label_files = [
                (
                    x.replace("images/train", "labels_with_ids/train")
                    .replace(".png", ".txt")
                    .replace(".jpg", ".txt")
                )
                for x in self.img_files
            ...
            # recognition  CHINESE LOWERCASE
            self.voc, self.char2id, self.id2char = get_vocabulary(
                "LOWERCASE", use_ctc=self.use_ctc
            )
            ...
            self.label_files = [
                (
                    (
                        "/mmu-ocr/weijiawu/Data/VideoText/MOTR/SynthText/labels_with_ids/train"
                        + x.split("/SynthText")[1]
                    )
                    .replace(".png", ".txt")
                    .replace(".jpg", ".txt")
                )
                if len(x.split("/SynthText")) > 1
                else (
                    x.replace("images", "labels_with_ids")
                    .replace(".png", ".txt")
                    .replace(".jpg", ".txt")
    ...
    def _pre_single_frame(self, idx: int):
        ...
        if osp.isfile(label_path):
            ...
            if size == 0:
                ...
                for i, line in enumerate(lines):
                    ...
                    gt_word = np.full(
                        (self.max_word_len,), self.char2id["PAD"], dtype=np.int
                    )
        ...
        targets["texts_ignored"] = torch.as_tensor(
            targets["texts_ignored"], dtype=torch.long
        )
        ...
        targets["upright_box"] = torch.as_tensor(
            targets["upright_box"], dtype=torch.float32
        ).reshape(-1, 4)
    ...
    def __getitem__(self, idx):
        ...
        if self.vis:
            ...
            for idx1, (img, ann) in enumerate(zip(images, targets)):
                ...
                imge = (
                    imge.cpu().numpy() * [0.229, 0.224, 0.225] + [0.485, 0.456, 0.406]
                ) * 255
                h, w = imge.shape[:2]
                label = ann["labels"]
                area = ann["area"]
                ...
                for obj_id, box, rotate, word, texts_ignored in zip(
                    obj_ids, boxes, rotates, words, texts_ignoreds
                ):
                    x_min, y_min, x_max, y_max = box.cpu().numpy()
                    #                     print(x_min,y_min,x_max,y_max)
                    rotate_mat = get_rotate_mat(-rotate)
                    temp_x = (
                        np.array([[x_min, x_max, x_max, x_min]]) - (x_min + x_max) / 2
                    )
                    temp_y = (
                        np.array([[y_min, y_min, y_max, y_max]]) - (y_min + y_max) / 2
                    )
...
def build_dataset2transform(args, image_set):
    ...
    dataset2transform_train = {
        "ICDAR2015": mot17_train,
        "DSText": mot17_train,
        "VideoSynthText": mot17_train,
        "FlowText": mot17_train,
        "FlowTextV2": mot17_train,
        "YVT": mot17_train,
        "BOVText": BOVText_train,
        "SynthText": crowdhuman_train,
        "UnrealText": crowdhuman_train,
        "COCOTextV2": crowdhuman_train,
        "VISD": crowdhuman_train,
        "FlowImage": crowdhuman_train,
        "ICDAR15_Pre": crowdhuman_train,
        "Synthetic_Chinese": synthetic_chinese_train,
    }

    dataset2transform_val = {
        "ICDAR2015": mot17_test,
        "DSText": mot17_test,
        "VideoSynthText": mot17_test,
        "FlowText": mot17_test,
        "YVT": mot17_test,
        "FlowTextV2": mot17_test,
        "BOVText": mot17_test,
        "SynthText": mot17_test,
        "UnrealText": mot17_test,
        "COCOTextV2": mot17_test,
        "VISD": mot17_test,
        "FlowImage": mot17_test,
        "ICDAR15_Pre": crowdhuman_train,
    }
