...
def parse_args():
    ...
    parser.add_argument(
        "--tests",
        type=str,
        default="/share/wuweijia/Code/VideoSpotting/MOTR/exps/e2e_TransVTS_r50_ICDAR15/e2e_json",
...
def iou_matrix_polygen(
    objs, gt_transcription, hyps, transcription, voc_list, max_iou=1.0
):
...
class Evaluator(object):
    ...
    def eval_frame(
        self, frame_id, trk_tlwhs, trk_ids, trk_transcription, rtn_events=False
    ):
        # results
        ...
        for gt in gt_objs:
            ...
            if (
                "#" in gt["transcription"]
                or len(gt["transcription"]) < 3
                or is_valid
                or gt["transcription"] == "555"
            ):
        ...
        iou_distance = iou_matrix_polygen(
            gt_tlwhs,
            gt_transcription,
            trk_tlwhs,
            trk_transcription,
    ...
    def eval_file(self, filename, transcription_path):
        ...
        for frame_id in range(len(self.gt_frame_dict)):
            ...
            self.eval_frame(
                str(frame_id), trk_tlwhs, trk_ids, trk_transcription, rtn_events=False
            )
...
def main():
    ...
    for seq in tqdm(filter_seqs):  # tqdm(seqs):
        ...
        transcription_path = os.path.join(
            args.tests.replace("e2e_json", "e2e_xml"), res_name.replace("json", "txt")
        )
