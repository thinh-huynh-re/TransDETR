...
if __name__ == "__main__":
    ...
    for cls in tqdm(os.listdir(frame_path_root)):
        ...
        for video in tqdm(os.listdir(frame_path)):
            if ".ipynb" in video:
                continue
            annotation_path_cls_v = os.path.join(
                annotation_path, video.split(".mp4")[0] + "_GT.xml"
            )
            frame_path_cls_v = os.path.join(frame_path, video.split(".mp4")[0])
            result_path_cls_video = os.path.join(
                result_path_cls, video.split(".mp4")[0]
            )
            ...
            for idx, frame_id in tqdm(enumerate(annotation.keys())):
                frame_vis_path = os.path.join(
                    result_path_cls_video, str(frame_id) + ".jpg"
                )
                ...
                for data in annotatation_frame:
                    ...
                    points = np.array(
                        [[x1, y1], [x2, y2], [x3, y3], [x4, y4]], np.int32
                    )
