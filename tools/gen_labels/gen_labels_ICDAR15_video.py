...
def parse_xml(annotation_path, image_path):
    ...
    for idx, child in enumerate(root):
        bboxes, IDs, rotates, words, orignial_bboxes = getBboxesAndLabels_icd13(
            height, width, child
        )
...
def gen_data_path(
    path, split_train_test="train", data_path_str="./datasets/data_path/ICDAR15.train"
):
    ...
    for video_name in os.listdir(image_path):
        ...
        for i in range(1, len(frame_list) + 1):
            frame_real_path = (
                "ICDAR2015/images/train/" + video_name + "/{}.jpg".format(i) + "\n"
            )
...
for seq in tqdm(seqs):
    ...
    bboxess, IDss, rotatess, wordss, orignial_bboxess = parse_xml(
        ann_path, osp.join(image_path_frame, "{}.jpg".format(1))
    )
    ...
    for i in range(len(IDss)):
        ...
        if IDss[i] == []:
            with open(label_fpath, "w") as f:
                ...
                continue
        #                 f.write(label_str)
        for bboxes, IDs, rotates, word, orignial_bboxes in zip(
            bboxess[i], IDss[i], rotatess[i], wordss[i], orignial_bboxess[i]
        ):
            track_id = int(IDs)
            x, y, w, h = bboxes
