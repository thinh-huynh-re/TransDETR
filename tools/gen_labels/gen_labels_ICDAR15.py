...
def gen_data_path(
    ...
    data_path_str="./datasets/data_path/ICDAR15_Pre.train",
):
    ...
    for video_name in os.listdir(image_path):
        ...
        for i in frame_list:
            frame_real_path = (
                "ICDAR15_Pre/images/train/" + video_name + "/{}".format(i) + "\n"
            )
            lines.append(frame_real_path)
    write_lines(data_path_str, lines)


from_label_root = "/share/wuweijia/Data/ICDAR2015/train_gt"
seq_root = "/share/wuweijia/Data/VideoText/MOTR/ICDAR2015_Pre/images/train"
label_root = "/share/wuweijia/Data/VideoText/MOTR/ICDAR2015_Pre/labels_with_ids/train"
mkdirs(label_root)
seqs = [s for s in os.listdir(seq_root)]

# gen_data_path(path = "/share/wuweijia/Data/VideoText/MOTR/ICDAR15_Pre")
...
for seq in seqs:
    ...
    ann_list = os.listdir(from_label_root)

    for a in tqdm(ann_list):
        ann_path = os.path.join(from_label_root, a)
        boxs = []
        ...
        for line in lines:
            ...
            boxs.append(box)
        one_imgs_path = os.path.join(
            image_path_frame, a.replace("gt_", "").replace("txt", "jpg")
        )
        ...
        label_fpath = osp.join(
            seq_label_root,
            a.replace("gt_", "").replace("jpg", "txt").replace("png", "txt"),
        )
        ...
        for box in boxs:
            tid_curr += 1
            points = np.array([int(i) for i in box])
            try:
