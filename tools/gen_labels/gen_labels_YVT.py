...
def parse_xml(annotation_path, image_path):
    ...
    for idx, child in enumerate(root):
        bboxes, IDs, rotates = getBboxesAndLabels_icd13(height, width, child)
...
def gen_data_path(
    path, split_train_test="train", data_path_str="./datasets/data_path/YVT.train"
):
    ...
    for video_name in os.listdir(image_path):
        ...
        for i in range(0, len(frame_list)):
            frame_real_path = (
                "YVT/images/train/"
                + video_name
                + "/{}f{}.jpg".format(video_name, str(i).zfill(4))
                + "\n"
            )
...
for seq in tqdm(seqs):
    ...
    if "ipynb" in seq:
        continue
    bboxess, IDss, rotatess = parse_xml(
        ann_path, osp.join(image_path_frame, os.listdir(image_path_frame)[0])
    )
    ...
    for i in range(len(IDss)):
        ...
        label_fpath = osp.join(
            seq_label_root, "{}f{}.txt".format(seq, str(frame_id).zfill(4))
        )
        frame_path_one = osp.join(
            image_path_frame, "{}f{}.jpg".format(seq, str(frame_id).zfill(4))
        )
