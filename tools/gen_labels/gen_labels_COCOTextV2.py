...
def gen_data_path(
    ...
    data_path_str="./datasets/data_path/COCOTextV2.train",
):
    ...
    for video_name in os.listdir(image_path):
        ...
        for i in frame_list:
            label_path = "/share/wuweijia/Data/VideoText/MOTR/COCOTextV2/labels_with_ids/train/train_image/" + i.replace(
                "jpg", "txt"
            ).replace(
                "png", "txt"
            )
            ...
            frame_real_path = (
                "COCOTextV2/images/train/" + video_name + "/{}".format(i) + "\n"
            )
...
for seq in seqs:
    ...
    for idx, a in tqdm(enumerate(gt["imgs"])):
        ...
        label_fpath = osp.join(
            seq_label_root,
            one_imgs["file_name"].replace("jpg", "txt").replace("png", "txt"),
        )
        ...
        for box in boxs:
            ...
            try:
                x1, y1, w1, h1 = cv2.boundingRect(
                    points.reshape((int(len(points) / 2), 2))
                )
